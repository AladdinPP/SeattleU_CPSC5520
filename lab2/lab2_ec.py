...
Usage:      python3 lab2.py GCDHOST GCDPORT SUID [DOB]
            - GCDHOST:  The hostname or IP address of the Group Coordination
                        Daemon (GCD).
            - GCDPORT:  The port number of the GCD.
            - SUID:     A Seattle University ID (should be in the range 1000000
                        to 9999999).
            - DOB:      (Optional) Mother's birthday in the format mm-dd.
                        Defaults to "01-01".

            Example:
            python3 lab2.py localhost 8080 1234567 05-10

            This command will start a node in the bully algorithm, registering
            it with the GCD, and initiating an election if necessary.
Input:      - Command-line arguments:
              ...
              4. DOB:       (Optional) Mother's birthday (mm-dd), used to
                            calculate days until the next birthday. Defaults
                            to "01-01" if not provided.
...
Extra Credit Feature: PROBE Messages
------------------------------------
This program includes an implementation of the PROBE feature as specified for
extra credit.

**Claiming Extra Credit: Implemented PROBE Messages**

The PROBE message is sent occasionally to the group leader by all the other
members of the group. The purpose of the PROBE message is to check the health
of the leader node. If the leader fails to respond to a PROBE message within
a specified timeout, the node that sent the PROBE will initiate a new election
after re-registering with the GCD to get the latest list of group members.

Implementation Notes:
  - Each node maintains the identity and address of the current leader.
  - A background thread (`probe_leader`) runs on each non-leader node to handle
    the periodic sending of PROBE messages.
  - The leader node handles incoming PROBE messages by responding with an `'OK'`.
  - All communication related to PROBE messages is logged with timestamps and
    peer identities.
"""
...
class Lab2TCPServer(socketserver.ThreadingTCPServer):
    ...
    def __init__(self, server_address, RequestHandlerClass, gcd_host, gcd_port,
                 identity):
        """
        ...
        self.leader_address = None  # Initialize leader address
    ...
    def start_election(self):
        ...
        with self.lock:
            ...
            higher_members = {ident: addr for ident, addr in self.members.items()
    ...
    def handle_election(self, sender_identity, sender_address, sender_members):
        ...
        Args:
            sender_identity (tuple):    The identity of the node that sent the
                                        election message.
    ...
    def declare_leader(self):
        ...
        with self.lock:
            ...
            self.leader_address = self.server_address  # Our own address
            self.in_election = False
        ...
        for ident, addr in self.members.items():
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(5)
                    s.connect(addr)
                    coordinator_message = ('COORDINATOR', self.identity, self.server_address)
    ...
    def handle_leader(self, new_leader_identity, new_leader_address):
        """
        ...
        with self.lock:
            ...
            self.in_election = False
            logging.info(f"Updated leader to {new_leader_identity} at {new_leader_address}")
    ...
    def start_probe(self):
        """
        Start a background thread that periodically probes the leader.
        """
        probe_thread = threading.Thread(target=self.probe_leader,
                                        name=f"ProbeThread-{self.identity}")
        probe_thread.daemon = True
        probe_thread.start()
        self.probe_thread = probe_thread  # Keep a reference if needed

    def probe_leader(self):
        """
        Periodically send PROBE messages to the leader.
        """
        while True:
            # Sleep for a random time between 500ms and 3000ms
            sleep_time = random.uniform(0.5, 3.0)
            time.sleep(sleep_time)
            with self.lock:
                if self.current_leader == self.identity:
                    # We are the leader, no need to probe
                    continue
                leader_identity = self.current_leader
                leader_address = self.leader_address  # Need to maintain this
                if not leader_identity or not leader_address:
                    # No leader known, skip probing
                    continue
            # Attempt to send PROBE message to leader
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(5)
                    s.connect(leader_address)
                    probe_message = ('PROBE',)
                    s.sendall(pickle.dumps(probe_message))
                    logging.info(
                        f"Sent PROBE message to leader {leader_identity} at {leader_address}")
                    # Wait for response
                    response = s.recv(4096)
                    response_data = pickle.loads(response)
                    if response_data == 'OK':
                        logging.info(
                            f"Received OK from leader {leader_identity}")
                    else:
                        logging.error(
                            f"Unexpected response from leader {leader_identity}: {response_data}")
            except Exception as e:
                logging.error(
                    f"Failed to send PROBE to leader {leader_identity} at {leader_address}: {e}")
                # Failed PROBE, re-register with GCD and start election
                self.connect_to_gcd()  # Re-register with GCD to get latest members
                self.start_election()

    def handle_probe(self):
        """
        Handle a PROBE message from a member by responding with 'OK'.
        """
        logging.info("Received PROBE message")
        return 'OK'

    def handle_request(self, message, client_socket, client_address):
        """
        ...
        if message_name == 'ELECTION':
            ...
            sender_address = message[2].get(sender_identity)
            self.handle_election(sender_identity, sender_address, message[2])
            return None
        elif message_name == 'COORDINATOR':
            new_leader_identity = message_data
            new_leader_address = message[2]
            self.handle_leader(new_leader_identity, new_leader_address)
            return None
        elif message_name == 'OK':
            self.handle_ok_received()
            return None
        elif message_name == 'PROBE':
            response = self.handle_probe()
            return response
        else:
...
class Lab2TCPRequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            ...
            response = self.server.handle_request(message, self.request,
                                                  peer_address)
            if response is not None:
                self.request.sendall(pickle.dumps(response))
                logging.info(f"Sent response to {peer_address}")
        except Exception as e:
            logging.error(
...
if __name__ == "__main__":
    ...
    server = Lab2TCPServer((HOST, PORT), Lab2TCPRequestHandler,
                           gcd_host, gcd_port, identity)
